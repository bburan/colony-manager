...
`/help/` serves user documentation written as Markdown in `colony_manager_gui/help/*.md`, one file per page type, each with a small `key: value` front-matter block (`title`, `section`, `order`, `summary`, `see_also`). `helpdocs.py` loads them and renders them with a **hand-rolled Markdown subset** — headings, paragraphs, nested lists, fenced code, blockquotes, pipe tables, inline emphasis/code/links, everything escaped first. That is deliberate: `docker-entrypoint.sh` reinstalls with `pip install --no-deps` at container start, so a new runtime dependency needs an image rebuild to land, which help text does not justify. Don't swap in `markdown`/`mistune` without also changing the deploy story.

Each page template fills `{% block header_help %}` with `macros.help_button('<slug>')`, which opens the topic in `#helpModal` via `openHelpModal()` (a plain `fetch`, kept clear of the `#modalBody` / `editModal` htmx flow so help can be opened from inside a form modal). `tests/test_helpdocs.py` asserts every `help_button` slug, every `see_also`, and every in-body `/help/<slug>` link resolves — a topic renamed without updating its referrers fails there rather than 404-ing for a user.

The host registry may also export `HELP_TOPICS` (see `datatypes.get_registry_help_topics`), so the deployment's data plugin documents its own experiment types; those merge into the index labelled *plugin*, and a description class's `help_topic` attribute puts a `?` button on every file of that type. Built-in slugs win a collision. `mmm_db` uses both — `mmm_db/helptopics.py` plus `mmm_db/help/*.md`.
