...
@breeding_bp.route('/')
def list_breeding_pairs() -> Response | str:
    pairs = db.session.scalars(
        select(BreedingPair).order_by(
            BreedingPair.is_active.desc(), BreedingPair.id.desc(),
        )
    ).all()
    return render_template('breeding.html', pairs=pairs)

@breeding_bp.route('/<int:breeding_pair_id>')
def view_breeding_pair(breeding_pair_id) -> Response | str:
...
def create_breeding_pair() -> Response | str:
...
def deactivate_breeding_pair(breeding_pair_id) -> Response | str:
    breeding_pair = get_or_404(BreedingPair, breeding_pair_id)
...
def reactivate_breeding_pair(breeding_pair_id) -> Response | str:
...
def create_litter(breeding_pair_id) -> Response | str:
    pair = get_or_404(BreedingPair, breeding_pair_id)
    form = LitterForm()
    if form.validate_on_submit():
...
def update_litter(litter_id) -> Response | str:
...
def delete_litter(litter_id) -> Response | str:
    litter = get_or_404(Litter, litter_id)
...
def wean_litter(litter_id) -> Response | str:
    litter = get_or_404(Litter, litter_id)
    form = WeaningForm()
    if form.validate_on_submit():
...
def create_breeding_pair_modal() -> Response | str:
...
def create_litter_modal(breeding_pair_id) -> Response | str:
    pair = get_or_404(BreedingPair, breeding_pair_id)
...
def edit_litter_modal(litter_id) -> Response | str:
    litter = get_or_404(Litter, litter_id)
...
def delete_litter_modal(litter_id) -> Response | str:
    litter = get_or_404(Litter, litter_id)
...
def wean_litter_modal(litter_id) -> Response | str:
    litter = get_or_404(Litter, litter_id)
